...
4. ⚠️ 同一張排班綁的所有項目共用其時段——**一般項目與二林模式項目不可同排班**（誤將細時段人數填 >0 到一般項目排班會讓一般項目出現細時段）。**2026-07-22 起此規則已由系統硬擋**（非僅人工 SOP）：`RosterAdminService.ValidateModeNotMixedAsync` 在自動配號分院建立/編輯排班時，若同時填了「配號時段」與「現場取號時段」的人數即拒絕（`ROSTER_MODE_MIXED`），前端 roster-form 送出前亦擋下。見 [design/frontend-backend.md](../design/frontend-backend.md) §模式感知呈現「混掛防呆」。
