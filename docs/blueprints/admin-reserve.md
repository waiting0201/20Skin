...
- **初診判斷（`IsFirstVisit`）用「分頁後單次 IN 清單 group-by」批次計算（2026-07-22 由相關子查詢改為此法）**：先取回本頁 ≤50 列，收集其 `MemberID`，再 `SELECT MemberID, COUNT(*) FROM Appointments WHERE MemberID IN @ids AND Status=1 GROUP BY MemberID` 一次算完（≤1 即初診），與 `ExportCheckinAsync` 同一範式。**原用相關子查詢投影，在無索引的 reused DB 上會為每頁列各觸發全表掃描，使列表 CPU 近乎翻倍（實測 189ms→75ms）**；IN 版本是單一查詢、非 N+1。判斷全域（跨分院/診別）累計次數，非本次查詢範圍限定，忠於舊系統 `appointmentsService.Get().Where(a => a.Status==1)` 無 Branch/Clinic 篩選的既有語意。動態計算不讀既有 `Appointments.IsFirstVisit` 欄位（該欄位存在但列表/匯出皆不使用，忠於舊系統既有行為）。
- **列表查詢效能索引（2026-07-22，DB 擁有者放行的政策例外）**：Appointments/Members 原僅 GUID 叢集 PK，`ListAsync` 每次載入對 Appointments 做約 4 次全表掃描（≈12,644 logical reads）。已加四索引 `Appointments(BranchID, AppointmentDate)`、`Appointments(MemberID, Status)`、`Members(Number)`、`Members(Mobile)`（腳本 `scripts/db/2026-07-22-add-appointment-indexes.sql`），降至 ≈1,200 reads（約 10 倍）。**配套鐵則**：COUNT 與分頁主查詢含 `(@dateOnly IS NULL OR …)` 萬用 predicate，光加索引無效，兩條已同步加 `OPTION (RECOMPILE)` 才會 seek（COUNT 3161→3、主查詢 3161→302）；初診 group-by／容量表 predicate 全為無條件等值/IN，免 RECOMPILE。索引與 RECOMPILE 必須同一次部署。詳見 [database-design.md](../design/database-design.md) §核心原則·例外、[gotchas.md](../gotchas.md)。
