...
- **時段「人數」語意澄清（2026-07-22）**：時段表單的人數（`Periods.Patients`）與排班的人數（`RosterPeriods.Patients`）角色不同——**客戶預約時真正的容量上限是排班人數**（`BookingService`/`AppointmentService` 皆讀 `RosterPeriods.Patients`，且客戶只能對有排班的時段預約）；時段人數在新系統只是「預設值／fallback」（新增排班目前預設 0、不從時段帶入；唯一仍消費它的是後台預約管理容量表在「當天無對應排班」時退回顯示 `Periods.Patients`，見 `AppointmentAdminService.cs:102`）。因 reused DB 欄位不可刪、NOT NULL、且 fallback 仍在用，**保留欄位**，僅在 `period-form.ts` 人數欄加說明「此為預設值；客戶實際可約人數以『門診管理』的排班人數為準」。（對照「起始編號」相反：模式權威在時段 `Periods.StartNumber`、排班那份休眠——即「人數改排班、配號/起始編號改時段」。）
- **排班容量表（`roster-form.ts`）分組＋SOP＋修 bug**：自動配號分院把各時段容量表分「配號時段（早/晚診）／現場取號時段」兩區，並加 ⚠️ 簡短提醒「一張排班只能填一種：配號 或 現場取號」。**順帶修 bug**：新增排班時「起始號碼」欄原本每列寫死 `null` 恆顯示 `—`（看不出模式），改為顯示 `Periods` 模板值（新增 `templateStartNumber` display-only 欄位）；**送出的 `RosterPeriods.StartNumber` 寫入語意完全不變**（新增仍送 `null`、編輯仍送既有值），避免誤啟用 RosterPeriods 的 StartNumber 覆寫——模式權威來源恆為基礎資料 `Periods`。
- **混掛防呆（2026-07-22 追加，前後端雙層）**：查證確認「一張台中健保排班同時填『配號時段』與『現場取號時段』的人數」**資料不會壞**（兩類各自獨立可訂、容量與門診號皆以 `PeriodID` 為鍵互不干擾），**但客戶端會語意混亂**——`appointment-form.ts` 的 `periodSectionTitle` 用 `some(有 outpatientTimeTitle)` 判斷，只要有配號時段標題就變「選擇早晚診」，而時段按鈕是單一扁平迴圈依 `rp.Sort` 輸出、無分組，於是早/晚診按鈕與「10:00」細時段按鈕交錯混排（即 SOP 的「細時段漏進一般項目」）。業務上一張排班只服務一種模式（時段全項目共用），混填永遠是錯誤。使用者裁示「介面分流＋說明簡單明白＋後端也要檢查」，故：
  - **後端硬擋**（`RosterAdminService.ValidateModeNotMixedAsync`，Create + Update）：僅自動配號分院；取人數>0 的 period 由 `Periods.StartNumber` 模板值判模式（**非送出的 `RosterPeriods.StartNumber`，後者新增恆 null**），若同時有配號（StartNumber 有值）與現場取號（null）→ 丟 `BusinessException("…請分開排班", "ROSTER_MODE_MIXED")`。Update 先 `SELECT BranchID FROM Rosters` 取分院。二林（`IsAutoRowNumber=false`）early-return 不受影響。
