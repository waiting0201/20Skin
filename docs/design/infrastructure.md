...
| GitHub Actions 首次部署：兩個 SPA 皆失敗 `Failed to find a default file in the app artifacts folder (web-customer)` | `Azure/static-web-apps-deploy@v1` 搭配 `skip_app_build: true` 且 `app_location` 非 repo 根目錄（monorepo 常見寫法）時，已知會忽略 `output_location`，直接把 `app_location` 本身當產物目錄找 `index.html` | 兩個 workflow 改成 `app_location: '/'`（repo 根目錄）+ `output_location` 改用「相對於 repo 根目錄」的完整路徑（如 `web-customer/dist/web-customer/browser`），繞開這個問題；API 部署（`deploy-api.yml` 不用這個 action）不受影響，首次即部署成功 |
