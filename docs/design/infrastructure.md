...
後台 SPA (SWA Free)      ─┘                                   ├─ Managed Identity ─→ Azure SQL 20Skin（reused，schema 不可改）
                                                               ...
                                                               ├─→ 智邦 SMS API（HTTPS；已實作，受總開關 Sms:Enabled 控制，正式預設停用）
                                                               ├─ 讀機密 ─→ Azure Key Vault（RBAC）
                                                               └─ Timer trigger（已實作，每日 08:00）→ 發待發 SmsStatus
...
| `Sms-ApiKey` | `Sms:ApiKey`（`Sms__ApiKey` KV reference，已掛載） | 智邦 SMS API 金鑰 | 開啟真發前必填（沿用舊智邦帳戶值）；未寫入時因 `Sms:Enabled=false` 走 NoOp 不影響啟動 |
| `Sms-Username` | `Sms:Username`（`Sms__Username` KV reference） | 智邦帳號 | 同上 |
| `Sms-Password` | `Sms:Password`（`Sms__Password` KV reference） | 智邦密碼 | 同上 |
...
**正式環境驗證腳本**：[`scripts/smoke/`](../../scripts/smoke/README.md)（`check_migrated_files.py`
驗收舊檔搬遷可顯示、`check_new_upload.py` 驗收新上傳可用+自動清理零殘留），2026-07-04 產出、
尚未對正式站實跑，詳見 [blueprints/file-upload.md](../blueprints/file-upload.md) §Smoke-test 腳本。

## 排程（CheckSms 取代，已實作 2026-07-24）

`SmsReminderTimerFunction`：`[TimerTrigger("0 0 8 * * *")]`，依 App Setting `WEBSITE_TIME_ZONE=Asia/Taipei`
...
總開關之下，三種寄送類型各一個 App Setting（未設定＝`true`，維持原行為）；實際是否真發仍由總開關把關。
**改值需重新部署／改 App Setting 後重啟**（同總開關，非後台可調——決策 2026-08-02）。

| App Setting | 對應設定鍵 | 控制 | 正式現值 | `false` 時 |
|---|---|---|---|---|
| `Sms__ImmediateEnabled` | `Sms:ImmediateEnabled` | 建約當下的即時確認簡訊 | `'true'` | 不呼叫供應商，該列回寫 `Status='OFF'`（非 `null`，以免被當日 Timer 撈走補送） |
| `Sms__ReminderEnabled` | `Sms:ReminderEnabled` | Timer 的前一天提醒 | `'false'` | Timer 早退、不動任何列（可逆、無 backlog） |
| `Sms__CancelEnabled` | `Sms:CancelEnabled` | 取消預約時標記 `CANCEL` | `'false'` | 不標記；⚠️ 待發列不被攔下 |
...
| 部署成功、頁面可開（HTTP 200），但使用者實測瀏覽器 console 出現 `ERR_CONNECTION_REFUSED`（非 Bicep/workflow 問題，是既有前端專案設定缺陷） | 兩個 `angular.json` 的 `production` build configuration **缺少 `fileReplacements`**，導致 `--configuration production` 建置時仍使用預設 `environment.ts`（`apiBase: http://localhost:7071/api`），而非 `environment.prod.ts`——正式站因此對「訪客自己的電腦」打 `localhost:7071`，當然連不上 | 兩個 `angular.json` 補上 `fileReplacements: [{ replace: "src/environments/environment.ts", with: "src/environments/environment.prod.ts" }]`；已用 `npm run build -- --configuration production` 實測產物內容確認不再含 `localhost:7071`，且線上兩站的已部署 JS chunk 皆已改含 `func-20skin-api-prod.azurewebsites.net` |
...
- 智邦 SMS 引擎已實作（`ChiefTelSmsSender` + Timer），但真發總開關 `Sms__Enabled` 正式預設 `'false'`（走 NoOp）；開啟前需在 Key Vault 寫入 `Sms-ApiKey`/`Sms-Username`/`Sms-Password`（沿用舊智邦帳戶）並於 staging 驗證智邦回應/HTTPS，見 [blueprints/sms-reminder.md](../blueprints/sms-reminder.md) §待辦。
