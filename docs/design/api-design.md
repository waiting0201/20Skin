...
| `/api/rosters?branch=&clinic=&category=&date=&doctorId=` | GET | 可預約時段（JSON）。**帶 `doctorId` → 該指定醫師時段（IsAppointment=1）；不帶 → 不指定（IsAppointment=0）**。2026-07-01。⚠️ **客戶前台自 2026-08-03 起不再帶 `doctorId`**（指定醫師改臨櫃告知），參數保留以便復原 |
| `/api/rosters/doctors?...` | GET | 該日可指定醫師。⚠️ **端點保留但客戶前台自 2026-08-03 起不再呼叫**，見 [blueprints/customer-booking.md](../blueprints/customer-booking.md) §指定醫師：客戶前台已下架 |
...
**預約管理（已實作 Done 2026-07-03，[blueprints/admin-reserve.md](../blueprints/admin-reserve.md)）**：`admin/appointments/{ta|ch|ch-dentist}`(GET 列表含時段容量表)、`…/{id}`(GET 詳情/POST `{id}/cancel` 取消；詳情回應於 2026-08-03 擴充 9 個預約本身欄位 `appointmentDate`/`periodTitle`/`slotTitle`/`doctorName`/`outpatientNum`/`status`/`isFirstVisit`/`branchIsAutoRowNumber`/`createDate`，口徑與列表頁一致，屬新增欄位、對既有呼叫端相容)、`…/capacity`(PUT 容量批次更新)、`…/export/{checkin|questionnaire}`(GET 簽到單 Excel / 問卷 JSON) — 3 組瘦 proxy 對應 `TaAppointments`/`ChAppointments`/`ChDentistAppointments`。
