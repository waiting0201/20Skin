...
/// <summary>排班列表項。CategoryTitles 為該排班開放科別項目標題（依 Categorys.Sort 排序、逗號串接），對應舊系統「項目」欄。</summary>
public sealed record RosterListItemDto(Guid RosterId, DateTime RosterDate, Guid? DoctorId, string? DoctorName, int? OutpatientTimeId, string? OutpatientTimeTitle, bool IsAppointment, string CategoryTitles);
...
/// <summary>編輯排班請求（不含重複設定，僅編輯單一天；RosterDate 可修改，忠於舊系統 EditTaRosters 等的 TryUpdateModel 白名單）。</summary>
public sealed record RosterUpdateRequest(Guid? DoctorId, int? OutpatientTimeId, DateTime RosterDate, bool IsAppointment, List<Guid> CategoryIds, List<RosterPeriodInput> Periods);
