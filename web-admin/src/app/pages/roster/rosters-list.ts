...
import { RosterApiService, rosterLabel, rosterResourceKey } from '../../core/services/roster-api.service';
  ...
  template: `
    <div class="bg-white rounded shadow-sm border border-hairline">
      <div class="flex flex-wrap items-center justify-between gap-2 px-5 py-3 border-b border-hairline">
        <h1 class="text-base font-semibold text-ink"><i class="fa fa-calendar text-muted mr-2"></i>{{ pageLabel() }}</h1>
      ...
      <table class="w-full text-sm">
        <thead>
          <tr class="text-left text-muted border-b border-hairline bg-surface">
            ...
            <th class="px-5 py-2.5 font-medium w-auto">項目</th>
            <th class="px-5 py-2.5 font-medium text-center w-24">需預約</th>
        ...
        <tbody>
          @for (r of items(); track r.rosterId) {
            <tr class="border-b border-hairline hover:bg-surface">
              ...
              <td class="px-5 py-2.5 text-muted">{{ r.categoryTitles || '—' }}</td>
...
export class RostersListComponent {
  ...
  readonly pageLabel = computed(() => rosterLabel(this.branch(), this.clinic()));
